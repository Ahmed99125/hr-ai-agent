...
class BambooHRService {
    getApiKey() {
        ...
        if (!key)
            throw new Error("BAMBOOHR_API_KEY is not set.\n" +
    ...
    getSubdomain() {
        ...
        if (!subdomain)
            throw new Error("BAMBOOHR_SUBDOMAIN is not set.\n" +
    ...
    normalizeLocation(raw) {
        ...
        if (s.includes("dubai") || s.includes("uae"))
            return "dubai_plant";
        if (s.includes("alex") || s.includes("egypt") || s.includes("egp"))
            return "alexandria_hub";
        if (s.includes("amman") || s.includes("jordan") || s.includes("jor"))
            return "amman_center";
        if (s.includes("riyadh") || s.includes("ksa") || s.includes("saudi"))
            return "riyadh_hq";
    ...
    async getEmployee(employeeId) {
        const resp = await fetch(`${this.baseUrl}/employees/${employeeId}` +
            `?fields=firstName,lastName,department,jobTitle,hireDate,workEmail,` +
            `supervisorId,supervisor,country,location,nationality,customIqamaExpiryDate,currency`, { headers: this.getHeaders() });
        if (!resp.ok)
            throw new Error(`BambooHR: Employee ${employeeId} not found (HTTP ${resp.status})`);
    ...
    async getTimeOffBalance(employeeId) {
        const resp = await fetch(`${this.baseUrl}/employees/${employeeId}/time_off/calculator`, { headers: this.getHeaders() });
        if (!resp.ok)
            throw new Error(`BambooHR: Could not get leave balance for ${employeeId} (HTTP ${resp.status})`);
        const data = await resp.json();
    ...
    // ─── Request Time Off ─────────────────────────────────────────────────────
    async requestTimeOff(employeeId, leaveType, startDate, endDate, notes) {
        const resp = await fetch(`${this.baseUrl}/employees/${employeeId}/time_off/request`, {
            method: "POST",
    ...
    // ─── Update Leave Status ──────────────────────────────────────────────────
    async updateTimeOffStatus(requestId, status, note) {
        const resp = await fetch(`${this.baseUrl}/time_off/requests/${requestId}/status`, {
            method: "PUT",
            headers: this.getHeaders(),
            body: JSON.stringify({ status, note: note ?? "" }),
        });
        if (!resp.ok)
            throw new Error(`BambooHR: Failed to update leave request ${requestId} (HTTP ${resp.status})`);
        return true;
    }
    // ─── Create Onboarding Checklist ──────────────────────────────────────────
    async createOnboardingChecklist(employeeId, country) {
        ...
        if (country === "KSA") {
            baseItems.push({ id: "CHK-09", task: "Register in GOSI (social insurance)", completed: false, dueDate: due(30), assignedTo: "HR" }, { id: "CHK-10", task: "Verify Nitaqat / Saudization eligibility", completed: false, dueDate: due(7), assignedTo: "HR" });
        }
        else if (country === "UAE") {
            baseItems.push({ id: "CHK-09", task: "Register in DEWS / DIFC GPSS (if applicable)", completed: false, dueDate: due(30), assignedTo: "HR" });
        }
    ...
    async getAllEmployees() {
        ...
        if (!resp.ok)
            throw new Error(`BambooHR: Could not fetch employee directory (HTTP ${resp.status})`);
        const data = await resp.json();
